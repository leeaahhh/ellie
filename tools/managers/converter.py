...
class Domain(Converter[URL]):
    ...
    async def convert(self: "Domain", ctx: Context, page: page):
        if not re.match(URL_VALIDATION_PATTERN, page.url):
        ...
        if self.filter and (
            ...
            for filter in NSFW_FILTERS:
                if filter in (page.url.lower().replace("+", "").replace("%20", "")):
                    await page.close()
                    raise CommandError("The URL provided contains NSFW content!")
        return URL(page.url)
