...
class shiro(AutoShardedBot):
    ...
    async def setup_hook(self: "shiro"):
        ...
        logging.info("logging into %s", self.user)
        ...
        for category in Path("features").iterdir():
            ...
            try:
                ...
                logging.info("Loaded category %s", category.name)
    ...
    async def on_guild_join(guild: Guild):
        ...
        logging.info("Joined guild %s (%s)", guild, guild.id)

    @staticmethod
    async def on_guild_remove(guild: Guild):
        logging.info("Left guild %s (%s)", guild, guild.id)

    async def on_ready(self):
        logging.info("Logged in as %s (%s)", self.user, self.user.id)
        logging.info("Connected to %s guilds", len(self.guilds))
