...
class Miscellaneous(Cog):
    ...
    async def launch_browser(self: "Miscellaneous"):
    ...
    async def close_browser(self: "Miscellaneous"):
    ...
    async def avatar_update(self, before: User, after: User):
        ...
        if not response.ok:
            # log error
            return print(f"{response.text} - {response.status}")

        data = await response.json()

        await self.bot.db.execute(
            "INSERT INTO metrics.avatars (user_id, avatar, delete, timestamp) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id, delete) DO NOTHING",
            before.id,
            data["url"],
            data["delete_key"],
            utcnow(),
    ...
    async def wolfram(self: "Miscellaneous", ctx: Context, *, query: str):
        ...
        async with ctx.typing():
            ...
            embed.set_footer(
                text="Wolfram Alpha",
                icon_url="https://cdn.shiro.wtf/u/j2zpAG.png",
            )
        await ctx.send(embed=embed)
        ...
        *,
        ...
        async with ctx.typing():
            ...
            await page.setUserAgent(
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.360"
            )
            ...
            if isinstance(ctx.channel, TextChannel):
                ...
                if (
                    any(keyword in content for keyword in ["xxx", "hentai", "porn"])
